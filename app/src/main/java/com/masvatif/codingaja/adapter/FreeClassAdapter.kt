...
class FreeClassAdapter(private val context: Context, private val freeclass: List<FreeClass>, val listener: (FreeClass) -> Unit)
    : RecyclerView.Adapter<FreeClassAdapter.FreeClassViewHolder>(){

    class FreeClassViewHolder(view: View): RecyclerView.ViewHolder(view) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FreeClassViewHolder {
        return FreeClassViewHolder(
            LayoutInflater.from(context).inflate(R.layout.item_kelas_gratis, parent, false)
        )
    }

    override fun onBindViewHolder(holder: FreeClassViewHolder, position: Int) {
